...
function passwordStrength (passwordText) {
  ...
  for (let i = 0; i < passwordText.length; i++) {
    ...
    if (lowerCase.includes(character)) { totalPasswordPoints += pointsLowerCase }
    if (upperCase.includes(character)) { totalPasswordPoints += pointsUpperCase }
    if (numbers.includes(character)) { totalPasswordPoints += pointsNumber }
    if (specialCharacters.includes(character)) { totalPasswordPoints += pointsSymbol }
  }

  if (totalPasswordPoints === passwordText.length) { totalPasswordPoints = 1 } // Cuando todos los caracteres son minusculas y mayúsculas
